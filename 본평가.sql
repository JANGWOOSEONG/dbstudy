...
*/
--날짜를 만드는 함수             : TO_DATE
--날짜 형식을 변경하는 함수 이름 : TO_CHAR

SELECT TO_CHAR(BIRTHDATE, 'YYYY-MM-DD') AS 생일 -- 공백을 넣을때 따옴표" "표시가능
  FROM STUDENT;
...
SELECT S.SNAME, D.DNAME, P.PNAME
  FROM DEPARTMENT D INNER JOIN STUDENT S  --1 대 M 순서로 적기
