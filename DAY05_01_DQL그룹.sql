...
GROUP BY
    ...
    2. GROUP BY절에서 지정한 칼럼은 "반드시" SELECT절에 있어야한다.(필수 규칙)
*/
...
-- 2. 동일한 DEPARTMENT_ID로 그룹화 하여 FIRST_NAME과 DEPARTMENT_ID를 조회하시오. (실패하는 쿼리문)
SELECT FIRST_NAME, DEPARTMENT_ID  -- FISRT_NAME 칼럼이 GROUP BY절에 없기 때문에 실행되지 않는다.
  FROM EMPLOYEES
 GROUP BY DEPARTMENT_ID;
