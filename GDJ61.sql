...
-- 이름(TABLE_NAME)이 'EMP'인 테이블에 존재하는 인덱스의 이름(INDEX_NAME)과 해당 인덱스가 설정된 칼럼 이름(COLUMN_NAME)을 조회할 수 있는 쿼리문을 작성하시오.

--현재 접속한 사용자(USER)가 작성한 인덱스를 대상으로 조회하시오. (10점)
SELECT E.INDEX_NAME , E.COLUMN_NAME , E.EMP_USER
  FROM EMP_TABLE E INNER JOIN 
...
SELECT DEPT_NAME, EMP_NO, SALARY
  FROM DEPARTMENT_TBL
 WHERE DEPT_NAME = (SELECT MAX(SALARY)
                      FROM EMPLOYEE_TBL);
...
SELECT EMP_ID, EMP_NAME, DEPT_NO, DEPT_NAME, SALARY
  FROM EMPLOYEE_TBL
 WHERE EMP_ID IN (SELECT DEPT_NO
                    FROM DEPARTMENT_TBL
                   WHERE DEPT_NAME = '총무부');
...
CREATE TABLE BOARD (
    ...
    BOARD_TITLE VARCHAR2(1000 BYTE) NOT NULL UNIQUE,  
...
ALTER TABLE BOARD
    ADD CONSTRAINT PK_BOARDS PRIMARY KEY(BOARD_NO);
