...
SELECT BIRTHDATE
  FROM STUDENT
 WHERE TO_DATE(BIRTHDATE 'YY/MM/DD','YYYY-MM-DD'));
...
SELECT S.STUDNO, S.SNAME, P.PNAME D.DNAME
  FROM DEPARTMENT_TBL D INNER JOIN STUDENT_TBL S
    ON D.DEPT_NO = S.DEPT_NO INNER JOIN PROFESSOR_TBL P
    ON D.PROF_NO = P.PROF_NO;
...
SELECT DEPARTMENT_NO , STUDENT_NO , COUNT(*)
  FROM DEPARTMENT;
...
SELECT PROFESSOR_USERID, PNAME, SALARY
  FROM PROFESSOR_TBL
 ORDER BY SALARY ;
...
SELECT GRADE, HEIGHT, WEIGHT, AVG
  FROM STUDENT_TBL
 WHERE HEIGHT , WEIGHT >= (SELECT AVG( 
...
CREATE OR REPLACE FUNCTION GET_NO(STUD_USERID STUDENT_TBL.STUD_USERID%TYPE)
RETURN NUMBER  
IS
    STUDENTNO STUDENT_TBL.NO%TYPE;
BEGIN
    SELECT STUDNO
      INTO STUD_NO
      FROM STUDENT_TBL
     WHERE STUD_USERID = USERID;
    RETURN STUDENTNO;
END;    
  

SELECT DISTINCT GET_NO('jun123') AS 학번
  FROM STUDENT_TBL
 WHERE STUDENT_USERID = 'jun123'; 
